...
import CaseStudyImplemention from "./CaseStudyImplemention.jsx";
import CaseStudyResult from "./CaseStudyResult.jsx";
import CaseStudyConclusion from "./CaseStudyConclusion.jsx";

const CaseStudy = ({ data }) => {
  console.log(data);

  const introduction = data?.list?.find((item) => item.text === "Introduction");
  const problems = data?.list?.find(
    (item) => item.text === "Problem Statement"
  );
  const solution = data?.list?.find((item) => item.text === "Solution");
  const implementation = data?.list?.find(
    (item) => item.text === "Implementation"
  );
  const result = data?.list?.find((item) => item.text === "Results");
  const conclusion = data?.list?.find((item) => item.text === "Conclusion");
  ...
  useEffect(() => {
    ...
    result?.current?.addEventListener("scroll", (e) => {
      console.log(e);
    });
  }, [resultRef.current]);
  ...
  const [activeSection, setActiveSection] = useState(data?.list[0]?.text);
